...
[Falco](https://falco.org/) は、CNCF（Cloud Native Computing Foundation）のインキュベーションプロジェクトであり、コンテナランタイムセキュリティのデファクトスタンダードツールです。eBPF という技術を用いて、カーネルレベルでシステムコールを監視し、あらかじめ定義されたルールに違反する不審なアクティビティをリアルタイムで検知します。

##### 重要な前提：正当な運用と攻撃の区別

Kubernetes 運用のベストプラクティスでは、**本番環境の Pod への直接的なシェルアクセス（`kubectl exec`）は避けるべき**とされています。代わりに、以下の手段を推奨します。

- **ログ出力**：アプリケーションが標準出力にログを出力し、`kubectl logs` で確認
- **メトリクス監視**：Prometheus などのメトリクス監視ツールで運用状況を把握
- **リモートデバッグ**：デバッグプロトコルを用いた非対話的なトラブルシューティング
- **緊急時のデバッグ**：`kubectl debug` を使用し、監査ログに記録される形で対応

しかし現実には、多くの組織（特にスタートアップやレガシーシステム）では、運用チームが本番 Pod にシェルで入ることがまだ一般的です。そのため、Falco ルールを運用環境に適用する際には、**正当な `kubectl exec` と攻撃による不正なシェル起動を区別する仕組み**が必須です。
...
組織として以下のいずれかの方針を決定し、Falco ルールを設定します。

- **方針A：本番環境への直接シェルアクセスを禁止**
  - `kubectl exec` は開発環境・ステージング環境のみ許可
  - 本番環境では `kubectl debug` または `kubectl logs` のみ使用
  - すべてのシェル起動を検知（除外ルールなし）
  - 利点：シンプルで誤検知が少ない

- **方針B：本番環境への直接シェルアクセスを制限付きで許可**
  - 特定のユーザー（SRE チーム）のみ、事前申請と監査ログ記録の下で許可
  - ホワイトリスト化により、許可されたアクセスは除外
  - 許可されていないアクセスは全て検知
  - 利点：緊急対応時の柔軟性がある
...
- **アラート疲れの防止**：初期段階では、不完全なホワイトリストのため、多くの誤検知を招く可能性があります。段階的にルールを調整し、本番運用へ適応させることが重要です。
- **ログ保持**：Falco のアラートだけでなく、Pod 内のシステムコール全体をログとして保持し、インシデント発生時の詳細調査に備えます。
- **他のセキュリティ層との連携**：Falco による検知と並行して、ネットワークポリシー（NetworkPolicy）、Pod セキュリティポリシー（Pod Security Policy）など、他のセキュリティ層も強化することで、多層防御を実現します。
