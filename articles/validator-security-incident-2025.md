...
Private Repository の場合、「どうせ非公開だから」とセキュリティ意識が薄れてしまうエンジニアも少なくありません。しかし、今回の事件が示すように、ひとたび Token が漏洩すれば、攻撃者は Repository 内のすべてのコード、設定、履歴にアクセス可能になります。この前提を念頭に置いて、以降の漏洩パターンと対策を確認していきましょう。
...
6. **サードパーティ連携（OAuth/GitHub App/Webhooks）の侵害**
   ...
   → **対応する対策**：
   - 【予防層】サードパーティ連携対策（OAuth/GitHub App のベストプラクティス遵守）
   - 【被害軽減層】サードパーティ連携のセキュリティ強化（影響範囲の限定）

7. **コンテナ/イメージ/キャッシュ層に残置**
   コンテナイメージをビルドする際に、イメージのレイヤーやビルドキャッシュに Token が残ってしまい、後からそれを再利用した人に見られてしまうケース。

   → **対応する対策**：
   - 【予防層】コンテナ/イメージ/キャッシュ残置対策（マルチステージビルドと `--mount=type=secret`）
   - 【被害軽減層】イメージスキャンによる検知と対応
...
8. **チケット/チャット/スクリーンショット共有からの漏洩**
   ...
   → **対応する対策**：【予防層】チケット/チャット/スクリーンショット対策
...
2. 【被害軽減層】万が一漏洩してしまった場合の被害を最小限に
3. 【検知・対応層】漏洩の早期検知と迅速な対応
...
- **`.gitignore` と `pre-commit` フックを活用する**: そもそも Token やパスワードなどの秘匿情報を Git の管理対象に含めないように `.gitignore` を正しく設定し、さらに `gitleaks` のようなツールを `pre-commit` フックに仕込んで、コミットする前にローカルで検知・ブロックする仕組みも有効である。
- **環境変数ファイルの暗号化と `.env` ファイルの管理**: 開発環境で使用する `.env` ファイルなどには秘匿情報が含まれることが多いため、これらのファイルは Git で管理しないようにし、必要に応じて暗号化して保管する。また、本番環境では環境変数を直接設定するか、`dotenv` のようなツールで安全に管理する。

#### 2. **CI/CD ログ・アーティファクト対策**

- **ログに秘匿情報を出力しない**: GitHub Actions の `::add-mask::` というコマンドを使えば、特定の文字列をログ上で `***` のようにマスクできる。Token、パスワード、API キーなどの秘匿情報は絶対にログに出力してはいけない。また、デバッグでよく使う `set -x` のようなコマンドは、意図せず環境変数を展開してしまう可能性があるので、本番のワークフローでは使用を禁止するルールを設ける必要がある。
...
#### 6. **チケット/チャット/スクリーンショット対策（秘匿情報の共有禁止）**

- **とにかく貼らない、写さない**: これはもう運用と文化の話になるが、「秘匿情報は絶対にパブリックな場所へは共有しない」というルールを徹底する。Issue や Pull Request のコメント、Slack などのチャットツールに Token、パスワード、API キーなどを貼り付けてはいけない。
...
- **外部ActionのコミットSHA固定**
  ワークフローで利用するサードパーティ製の Action は、`actions/checkout@v4` のようにタグで指定するのではなく、`actions/checkout@a12a3943b4bde6ff22b8f99495b33c59aed7c3d2` のように**コミットハッシュで固定**します。これにより、Action の作者が悪意のあるバージョンをタグに上書きする、といったサプライチェーン攻撃のリスクを**予防**し、万が一 Action が乗っ取られた場合でも、意図しないコードが実行される**被害を軽減**できます。
